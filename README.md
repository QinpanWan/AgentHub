...
- **v1.2（2026-08-28）**：修复 Claude 对话超时与一键巡检报错，新增 `~/MEMORY.md` 共享记忆展示与任务自动记录：
  ...
  - 巡检卡死：只杀直子进程无法终止 npx→claude-code 子链，改为 detached + 进程组 SIGKILL
  - 启动崩溃：`server/config.js` 在 `HOME` 声明前引用（TDZ），无 `MEMORY_MD_FILE` 环境变量时启动即崩，已提前声明修复
  - 共享记忆：新增 `~/MEMORY.md` 展示，任务完成后自动追加任务记录（可配置 `memory.recordTasks=false` 关闭）
  - `self-test` 39/39 通过
- **v1.1**：新增团队共享上下文与共享记忆库（共享上下文自动注入、记忆按标签/关键词召回、共享记忆管理界面）
- **v1.0**：v1.0 版本正式发布
